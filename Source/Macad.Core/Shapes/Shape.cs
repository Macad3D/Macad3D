...
public abstract class Shape : Entity, IShapeOperand, IShapeDependent
{
    ...
    public virtual void InvalidateTransformation()
    {
        ...
        {
            _TransformedBRep.Location(new TopLoc_Location(GetTransformation()).Multiplied(_BRep.Location()));
            RaiseShapeChanged();
            if(IsVisible)
                Body?.RaiseVisualChanged();
        }
    ...
    public bool Make(MakeFlags flags)
    {
        ...
        if (IsValid)
        {
            ...
            if (IsValid)
            {
                // This is the case when triggering invalidation leads to recursively remaking the shape
                return true;
            }
        }
    ...
    public virtual SubshapeReference GetSubshapeReference(TopoDS_Shape ocSubshape)
    {
        ...
        SubshapeReference __FindReferenceInList(IList shapes)
        {
            ...
            else if (sameShapes.Length > 1)
            {
                ...
                // This can be the case when two subshapes are generated by using the same underlying TShape
                // The only way to differentiate is to check the (localSpace!) location, this can be done
                // using the hash code
