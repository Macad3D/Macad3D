...
namespace Macad.Test.Core.Modeling.Sketch;
...
public class SketchTests
{
    ...
    public void CorrectInvalidReferencesOnLoad()
    {
        ...
        var sketch = Context.Current.Document.FindInstances<Macad.Core.Shapes.Sketch>().FirstOrDefault();
    ...
    Macad.Core.Shapes.Sketch CreateSketch(SketchType sketchType)
    {
        var sketch = new Macad.Core.Shapes.Sketch();
        Assert.IsNotNull(sketch);
